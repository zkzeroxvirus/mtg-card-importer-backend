...
const LOAD_LOCK_FILE = path.join(BULK_DATA_DIR, `${CARD_FILE_BASENAME}.load.lock`);
const LOAD_LOCK_STALE_MS = 30 * 60 * 1000; // 30 minutes
const LOAD_LOCK_POLL_MS = 1000; // 1 second
const LOAD_LOCK_MAX_POLL_MS = 10000; // 10 seconds
...
function tryAcquireLoadLock() {
  try {
    // Lock file format: line 1 = PID, line 2 = ISO timestamp.
    fs.writeFileSync(LOAD_LOCK_FILE, `${process.pid}\n${new Date().toISOString()}\n`, { flag: 'wx' });
    return true;
  } catch (error) {
    if (error.code === 'EEXIST') {
      return false;
...
async function waitForLoadLockRelease() {
  let pollDelayMs = LOAD_LOCK_POLL_MS;
  while (true) {
    let lockContents;
    try {
      lockContents = fs.readFileSync(LOAD_LOCK_FILE, 'utf8');
    } catch (error) {
      if (error.code === 'ENOENT') {
        break;
      }
    ...
    const lockAgeMs = Number.isNaN(lockTimestamp)
      ? LOAD_LOCK_STALE_MS + 1 // Treat invalid timestamps as stale.
      : Math.max(0, Date.now() - lockTimestamp);
    if (lockAgeMs > LOAD_LOCK_STALE_MS) {
      console.warn(`[BulkData] Stale load lock detected (older than ${LOAD_LOCK_STALE_MS / 60000} minutes), removing...`);
      try {
        fs.unlinkSync(LOAD_LOCK_FILE);
      } catch (error) {
        if (error.code !== 'ENOENT') {
    ...
    pollDelayMs = Math.min(pollDelayMs * 2, LOAD_LOCK_MAX_POLL_MS);
  }
}
...
async function loadBulkData() {
  ...
  let processLockAcquired = false;
  try {
    while (!tryAcquireLoadLock()) {
      console.log('[BulkData] Another process is loading bulk data, waiting...');
      await waitForLoadLockRelease();
    }
