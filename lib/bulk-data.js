...
function parseQuery(query) {
  ...
  if (!/[:=<>]/.test(query)) {
    const normalizedName = normalizeCardName(query).toLowerCase();
    if (normalizedName) {
  ...
  parts.forEach(part => {
    ...
    if (part.match(/^block:/i)) {
      ...
      if (match) filters.block = (match[1] || match[2]).toLowerCase();
    }
    ...
    if (part.match(/^(fo|fulloracle):/i)) {
      ...
      if (match) filters.fullOracle = (match[2] || match[3]).toLowerCase();
    }
...
function filterCards(cards, filters) {
  return cards.filter(card => {
    ...
    if (filters.fullOracle) {
      const oracleText = (card.oracle_text || '').toLowerCase();
      const flavorText = (card.flavor_text || '').toLowerCase();
      // Full oracle includes both oracle and flavor text
      if (!oracleText.includes(filters.fullOracle) && !flavorText.includes(filters.fullOracle)) return false;
    }
    ...
    if (filters.printsEquals !== null || filters.printsGreater !== null || filters.printsGreaterEqual !== null || 
        filters.printsLess !== null || filters.printsLessEqual !== null) {
    ...
    if (filters.isSpotlight) {
      const frameEffects = card.frame_effects || [];
      if (!frameEffects.includes('spree') && !card.story_spotlight) return false;
    }
      ...
      const isFetch = typeLine.includes('land') && 
                      (oracleText.includes('search your library for a') || 
                       ...
                       ['polluted delta', 'bloodstained mire', 'wooded foothills', 'windswept heath', 'flooded strand',
                        ...
                        'prismatic vista', 'fabled passage', 'evolving wilds', 'terramorphic expanse'].some(f => name.includes(f)));
    ...
    if (filters.isShockland) {
      ...
      const name = (card.name || '').toLowerCase();
      const typeLine = (card.type_line || '').toLowerCase();
      // Shocklands have two basic land types and deal 2 damage if not paid
      const isShock = typeLine.includes('land') && 
                      (oracleText.includes('2 damage') || 
                       ['temple garden', 'hallowed fountain', 'watery grave', 'blood crypt', 'stomping ground',
                        'godless shrine', 'steam vents', 'overgrown tomb', 'sacred foundry', 'breeding pool'].some(s => name.includes(s)));
