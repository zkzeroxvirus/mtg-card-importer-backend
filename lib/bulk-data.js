...
function buildCardIndexes(cards) {
  ...
  for (const card of cards) {
    ...
    const hasFaces = Array.isArray(card.card_faces) && card.card_faces.length > 0;
    if (hasFaces) {
      const faceMetadata = card.card_faces.map((face, index) => ({
        index,
        name: face.name || '',
      ...
      card._face_metadata = faceMetadata;
      card._front_face = faceMetadata[0] || null;
      card._back_face = faceMetadata[1] || null;
      card._has_multiface = faceMetadata.length > 1;
      card._front_face_colors = card._front_face ? card._front_face.colors.slice() : (card.colors || []).slice();
      card._back_face_colors = card._back_face ? card._back_face.colors.slice() : [];

      const mergedFaceColors = new Set();
      for (const face of faceMetadata) {
        for (const color of face.colors) {
          mergedFaceColors.add(color);
        }
      }
      card._merged_face_colors = Array.from(mergedFaceColors);
    } else {
      card._face_metadata = [];
      card._front_face = null;
      card._back_face = null;
      card._has_multiface = false;
      card._front_face_colors = (card.colors || []).slice();
      card._back_face_colors = [];
      card._merged_face_colors = (card.colors || []).slice();
    }

    card._color_identity_lc = (card.color_identity || []).map(c => String(c || '').toLowerCase());
...
function getCardByOracleId(oracleId) {
  ...
  if (cardsByOracleId.size > 0) {
    return cardsByOracleId.get(normalizedOracleId) || null;
  }
