...
/**
 * Check if card is a test card or has acorn security stamp (confidential)
 * Test cards are from Mystery Booster playtest sets (cmb1, mb2, cmb2)
 * Acorn stamped cards are not tournament legal
 */
function isTestOrAcornCard(card) {
  ...
  // Check for acorn security stamp (confidential/not tournament legal)
...
async function getRandomCard(query = '', suppressLog = false) {
  ...
  // Exclude test cards and acorn-stamped (confidential) cards from random results
  matchingCards = matchingCards.filter(card => !isTestOrAcornCard(card));
