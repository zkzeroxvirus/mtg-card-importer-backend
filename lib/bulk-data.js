...
async function downloadBulkFile(type, outputPath) {
  ...
  try {
    ...
    const contentEncoding = typeof bulkInfo.content_encoding === 'string'
      ? bulkInfo.content_encoding.toLowerCase()
      : '';
    if (!contentEncoding) {
      console.warn(`[BulkData] ${type} bulk data missing content_encoding; falling back to response headers.`);
    }
