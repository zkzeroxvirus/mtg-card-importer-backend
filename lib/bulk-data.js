...
function isExtraCard(card) {
  ...
  return /\b(token|emblem|scheme|plane|phenomenon|vanguard|conspiracy)\b/.test(typeLine);
}
...
async function getRandomCard(query = '', suppressLog = false) {
  ...
  const extraTypePattern = 'token|emblem|scheme|plane|phenomenon|vanguard|conspiracy';
  const extraTypeRequested = filters.type.some(type => extraTypePattern.split('|').includes(type)) ||
    new RegExp(`(?:^|[\\s(+])(?:t|type|is):(?:${extraTypePattern})\\b`, 'i').test(query);
