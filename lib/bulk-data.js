...
function parseQuery(query) {
  ...
  // Build regex from prefixes: matches "-?prefix[:=<>!]"
  const prefixPattern = STRUCTURED_PREFIXES.map(p => p.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|');
  const structuredFilterRegex = new RegExp(`^-?(${prefixPattern})[:=<>!]`, 'i');
  
  parts.forEach(part => {
    part = part.trim();
    
    // Check if this part is a structured filter
    const isStructuredFilter = structuredFilterRegex.test(part);
