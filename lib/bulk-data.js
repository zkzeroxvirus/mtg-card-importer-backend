...
function getCardByPartialName(name, set = null) {
  ...
  for (const [normalizedName, cards] of cardsByNormalizedName.entries()) {
    ...
    const hasTokenPrefix = normalizedName
      .split(' ')
    ...
    // Prefer tighter matches when multiple names contain the same fragment.
    score -= Math.max(0, normalizedName.length - normalizedQuery.length);
