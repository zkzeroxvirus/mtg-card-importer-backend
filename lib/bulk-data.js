...
function parseQuery(query) {
  ...
  parts.forEach(part => {
    ...
    if (part.match(/^(t|type):/i) && !part.match(/^t:id:/i)) {
      const match = part.match(/^(?:t|type):(\w+)/i);
      if (match) filters.type.push(match[1].toLowerCase());
    }
    
    if (part.match(/^-(t|type):/i)) {
      const match = part.match(/^-?(?:t|type):(\w+)/i);
      if (match) filters.excludeType.push(match[1].toLowerCase());
    }
    
    // is:token / is:emblem should behave like type filters
    if (part.match(/^is:(\w+)/i)) {
      const match = part.match(/^is:(\w+)/i);
      const extraType = match ? match[1].toLowerCase() : null;
      if (extraType && EXTRA_CARD_TYPE_SET.has(extraType)) {
