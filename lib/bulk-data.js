...
function parseQuery(query) {
  ...
  parts.forEach(part => {
    ...
    // Check if this part looks like a structured filter (starts with a known prefix or contains operators)
    // Known prefixes: s:, set:, r:, c:, color:, id:, identity:, t:, type:, is:, o:, mv:, cmc:, pow:, power:, 
    // tou:, toughness:, loy:, loyalty:, k:, keyword:, a:, artist:, ft:, flavor:, name:, produces:, game:, 
    // usd:, eur:, tix:, f:, legal:, banned:, restricted:, layout:, m:, mana:, has:, block:, fo:, fulloracle:, 
    // prints:, stamp:, frame:, b:, border:, year:, date:, -
    const isStructuredFilter = /^(-?)(s(et)?|r(arity)?|c(olor)?|id(entity)?|t(ype)?|is|o(racle)?|(mv|cmc|manavalue)|pow(er)?|tou(ghness)?|loy(alty)?|k(eyword)?|a(rtist)?|ft|flavor|name|produces|game|usd|eur|tix|f(ormat)?|legal|banned|restricted|layout|m(ana)?|has|block|fo|fulloracle|prints|stamp|frame|b(order)?|year|date)[:=<>!]/i.test(part);
