...
async function waitForDownloadLockRelease() {
  while (fs.existsSync(DOWNLOAD_LOCK_FILE)) {
    try {
      const stats = fs.statSync(DOWNLOAD_LOCK_FILE);
      if (Date.now() - stats.mtimeMs > DOWNLOAD_LOCK_STALE_MS) {
        console.warn('[BulkData] Download lock is stale, removing...');
        fs.unlinkSync(DOWNLOAD_LOCK_FILE);
        break;
      }
    } catch (error) {
    ...
    await new Promise(resolve => setTimeout(resolve, DOWNLOAD_LOCK_POLL_MS));
  }
}
