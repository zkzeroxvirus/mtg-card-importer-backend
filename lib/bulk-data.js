...
async function loadBulkData() {
  ...
  if (cardsFileExists && rulingsFileExists && !needsDownload) {
    try {
      ...
      if (fs.existsSync(DEFAULT_CARDS_FILE)) {
        fs.unlinkSync(DEFAULT_CARDS_FILE);
      }
  ...
  if (needsDownload) {
    ...
    // Recursively call to load the freshly downloaded files
    cardsLoaded = false;
    return loadBulkData();
  }
}
...
function getStats() {
  ...
  return {
    ...
    rulingsCount: rulingsDatabase ? rulingsDatabase.length : 0,
    cardsFileSize: fs.existsSync(DEFAULT_CARDS_FILE) ? fs.statSync(DEFAULT_CARDS_FILE).size : 0,
    rulingsFileSize: fs.existsSync(RULINGS_FILE) ? fs.statSync(RULINGS_FILE).size : 0,
    cardsModified: fs.existsSync(DEFAULT_CARDS_FILE) ? fs.statSync(DEFAULT_CARDS_FILE).mtime : null,
    rulingsModified: fs.existsSync(RULINGS_FILE) ? fs.statSync(RULINGS_FILE).mtime : null,
