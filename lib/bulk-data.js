...
let lastRandomPickByQuery = new Map();
...
function pickRandomIndexWithRepeatProtection(query, poolLength) {
  if (!Number.isInteger(poolLength) || poolLength <= 0) {
    return -1;
  }

  const queryKey = normalizeQueryForCache(query || '__default__');
  const previousIndex = lastRandomPickByQuery.get(queryKey);
  let randomIndex = getSecureRandomIndex(poolLength);

  // Avoid immediate repeats for a better perceived random experience.
  if (poolLength > 1 && Number.isInteger(previousIndex) && randomIndex === previousIndex) {
    randomIndex = getSecureRandomIndex(poolLength);
    if (randomIndex === previousIndex) {
      randomIndex = (previousIndex + 1) % poolLength;
    }
  }

  lastRandomPickByQuery.set(queryKey, randomIndex);
  return randomIndex;
}
...
const COLON_ONLY_PREFIXES = [
  ...
  'a', 'artist', 'ft', 'flavor', 'k', 'keyword', 'layout', 'wm', 'watermark',
...
async function getRandomCard(query = '', suppressLog = false) {
  ...
  if (filters.invalidOperatorParts.length > 0) {
    if (!suppressLog) {
      console.log(`[BulkData] Invalid operator in query: "${query}". Use ":" for keyword filters.`);
    }
    return null;  // Signal API fallback required
  }

  let matchingCards = getCachedQueryPool('random', query);
  if (!matchingCards) {
    ...
    const isTokenOrEmblemQuery = filters.type.includes('token') || filters.type.includes('emblem');

    // Exclude non-playable cards from random results unless the query explicitly targets token/emblem cards.
    const dedupeStart = Date.now();
    if (!isTokenOrEmblemQuery) {
      matchingCards = matchingCards.filter(card => !isNonPlayableCard(card));
    } else {
      // Match search behavior: avoid DFC token variants for token-specific lookups.
  ...
  const randomIndex = pickRandomIndexWithRepeatProtection(query, matchingCards.length);
