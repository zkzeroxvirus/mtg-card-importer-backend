...
const EXTRA_CARD_TYPES = ['token', 'emblem', 'scheme', 'plane', 'phenomenon', 'vanguard', 'conspiracy'];
const EXTRA_CARD_TYPE_PATTERN = EXTRA_CARD_TYPES.join('|');
const EXTRA_CARD_TYPE_REGEX = new RegExp(`(?:^|[\\s—])(${EXTRA_CARD_TYPE_PATTERN})(?:[\\s—]|$)`, 'i');
...
async function getRandomCard(query = '', suppressLog = false) {
  ...
  const extraTypeRequested = (filters.type || []).some(type => EXTRA_CARD_TYPES.includes(type)) ||
    EXTRA_TYPE_QUERY_REGEX.test(query);
  if (!extraTypeRequested) {
