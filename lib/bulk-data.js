...
let isReloading = false; // Mutex to prevent concurrent reloads
let isDownloading = false; // Mutex to prevent concurrent downloads
...
async function downloadBulkFile(type, outputPath) {
  ...
  try {
    ...
    console.log(`[BulkData] Compressing and saving ${type}...`);
    
    try {
      // Compress the data before writing (Scryfall returns uncompressed JSON)
      const compressedData = zlib.gzipSync(response.data);
      fs.writeFileSync(tempPath, compressedData);
...
async function downloadBulkData() {
  ...
  if (isDownloading) {
    console.log('[BulkData] Download already in progress, skipping...');
    return false;
  }
...
async function loadBulkData() {
  ...
  if (isReloading) {
    ...
    // Wait for the current reload to complete
    while (isReloading) {
