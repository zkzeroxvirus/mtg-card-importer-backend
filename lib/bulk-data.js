...
function filterCards(cards, filters) {
  return cards.filter(card => {
    ...
    if (filters.colorsCount !== null) {
      ...
      const cardColors = card.colors || [];
      // Query colors already pre-converted to uppercase in parseQuery
      const hasAll = filters.colorsUpper.every(c => cardColors.includes(c));
      const isSubset = cardColors.every(c => filters.colorsUpper.includes(c));
    ...
    if (filters.colorIdentityCount !== null) {
      ...
      if (op === '<' && count >= filters.colorIdentityCount) return false;
      if (op === '<=' && count > filters.colorIdentityCount) return false;
    } else if (filters.colorIdentityUpper && filters.colorIdentityUpper.length > 0) {
      const cardIdentity = card.color_identity || [];
      const hasAll = filters.colorIdentityUpper.every(c => cardIdentity.includes(c));
      const isSubset = cardIdentity.every(c => filters.colorIdentityUpper.includes(c));
      const isExact = hasAll && isSubset;
