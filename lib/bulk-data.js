...
function getRandomMatchingCards(query, suppressLog) {
...
async function getRandomCard(query = '', suppressLog = false) {
  ...
  const matchingCards = getRandomMatchingCards(query, suppressLog);
  if (!matchingCards) {
    return null;
  }
...
async function getRandomCards(query = '', count = 1, suppressLog = false) {
  const perfStart = Date.now();
  if (!cardsLoaded || !cardsDatabase) {
    if (!suppressLog) {
