...
async function loadBulkData() {
  ...
  if (cardsFileExists && !needsDownload) {
    try {
      ...
      cardsDatabase = await parseGzipJsonArray(CARD_DATA_FILE);
      ...
      if (INCLUDE_RULINGS && rulingsFileExists) {
        try {
          ...
          rulingsDatabase = await parseGzipJsonArray(RULINGS_FILE);
        } catch (rulingsError) {
...
/**
 * Parse a gzipped JSON array using streaming, avoiding a giant intermediate string.
 */
async function parseGzipJsonArray(filePath) {
