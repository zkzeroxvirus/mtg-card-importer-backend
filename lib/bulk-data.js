...
const CARD_DATA_FILE_UNCOMPRESSED = path.join(BULK_DATA_DIR, `${CARD_FILE_BASENAME}.json`);
const RULINGS_FILE = path.join(BULK_DATA_DIR, 'rulings.json.gz');
const RULINGS_FILE_UNCOMPRESSED = path.join(BULK_DATA_DIR, 'rulings.json');
...
// Performance: Indexes for O(1) lookups instead of O(n) scans
let nameIndex = null;        // Map<normalizedName, Card[]>
let oracleIdIndex = null;    // Map<oracleId, Card>
let rulingsIndex = null;     // Map<oracleId, Ruling[]>
 ...
 * @param {boolean} scheduleUpdates - Whether to schedule automatic updates (should be false in cluster workers)
 */
async function loadBulkData(scheduleUpdates = true) {
  ...
  try {
    ...
    const cardsUncompressedExists = fs.existsSync(CARD_DATA_FILE_UNCOMPRESSED);
    const rulingsFileExists = fs.existsSync(RULINGS_FILE);
    const rulingsUncompressedExists = fs.existsSync(RULINGS_FILE_UNCOMPRESSED);
  ...
  if ((cardsFileExists || cardsUncompressedExists) && !needsDownload) {
    try {
      console.log('[BulkData] Loading bulk data into memory...');
      const startTime = Date.now();
      
      // Load card data - prefer uncompressed cache for faster loading
      console.log('[BulkData] Loading card data...');
      let cardsJson;
      if (cardsUncompressedExists) {
        console.log('[BulkData] Using cached uncompressed card data...');
        cardsJson = fs.readFileSync(CARD_DATA_FILE_UNCOMPRESSED, 'utf8');
      } else {
        console.log('[BulkData] Decompressing card data (this may take a moment)...');
        cardsJson = await decompressFile(CARD_DATA_FILE);
        // Cache the decompressed data for faster loading by other workers
        console.log('[BulkData] Caching decompressed card data...');
        fs.writeFileSync(CARD_DATA_FILE_UNCOMPRESSED, cardsJson, 'utf8');
      }
      
      console.log('[BulkData] Parsing card data JSON (this may take a moment)...');
      cardsDatabase = JSON.parse(cardsJson);
      
      // Load and decompress rulings if enabled AND file exists
      if (INCLUDE_RULINGS && (rulingsFileExists || rulingsUncompressedExists)) {
        try {
          console.log('[BulkData] Loading rulings...');
          let rulingsJson;
          if (rulingsUncompressedExists) {
            console.log('[BulkData] Using cached uncompressed rulings...');
            rulingsJson = fs.readFileSync(RULINGS_FILE_UNCOMPRESSED, 'utf8');
          } else {
            console.log('[BulkData] Decompressing rulings...');
            rulingsJson = await decompressFile(RULINGS_FILE);
            // Cache the decompressed rulings
            fs.writeFileSync(RULINGS_FILE_UNCOMPRESSED, rulingsJson, 'utf8');
          }
      ...
      // Schedule next update check (only in primary process or non-cluster mode)
      if (scheduleUpdates) {
        scheduleUpdateCheck();
      } else {
        console.log('[BulkData] Skipping automatic update scheduling (cluster worker mode)');
      }
      ...
      // Delete corrupted files (both compressed and uncompressed)
      if (fs.existsSync(CARD_DATA_FILE)) {
        fs.unlinkSync(CARD_DATA_FILE);
      }
      if (fs.existsSync(CARD_DATA_FILE_UNCOMPRESSED)) {
        fs.unlinkSync(CARD_DATA_FILE_UNCOMPRESSED);
      }
      if (fs.existsSync(RULINGS_FILE)) {
        fs.unlinkSync(RULINGS_FILE);
      }
      if (fs.existsSync(RULINGS_FILE_UNCOMPRESSED)) {
        fs.unlinkSync(RULINGS_FILE_UNCOMPRESSED);
      }
    ...
    if (needsDownload) {
      ...
      // After successful download, decompress and cache for workers
      try {
        console.log('[BulkData] Decompressing freshly downloaded bulk data...');
        const cardsJson = await decompressFile(CARD_DATA_FILE);
        
        // Cache the decompressed data for faster loading by workers
        console.log('[BulkData] Caching decompressed card data for workers...');
        fs.writeFileSync(CARD_DATA_FILE_UNCOMPRESSED, cardsJson, 'utf8');
        
        console.log('[BulkData] Parsing card data JSON...');
        cardsDatabase = JSON.parse(cardsJson);
...
async function decompressFile(filePath) {
  return new Promise((resolve, reject) => {
    ...
    // 5 minutes should be more than enough for even large bulk data files
 ...
 * Called after loading card and rulings data into memory
 */
function buildIndexes() {
  ...
  const startTime = Date.now();
  console.log('[BulkData] Building performance indexes...');
  
  // Build name index: normalizedName -> Card[]
  nameIndex = new Map();
  for (const card of cardsDatabase) {
    const normalized = normalizeCardName(card.name || '').toLowerCase();
    if (normalized) {
  ...
  // Build oracle ID index: oracleId -> Card (for first printing lookup)
  oracleIdIndex = new Map();
  for (const card of cardsDatabase) {
    if (card.oracle_id && !oracleIdIndex.has(card.oracle_id)) {
      oracleIdIndex.set(card.oracle_id, card);
    }
  }
  
  // Build rulings index: oracleId -> Ruling[]
  if (rulingsDatabase) {
    rulingsIndex = new Map();
    for (const ruling of rulingsDatabase) {
      if (ruling.oracle_id) {
        if (!rulingsIndex.has(ruling.oracle_id)) {
          rulingsIndex.set(ruling.oracle_id, []);
        }
        rulingsIndex.get(ruling.oracle_id).push(ruling);
      }
    }
  } else {
    rulingsIndex = new Map();
  ...
  const indexTime = ((Date.now() - startTime) / 1000).toFixed(2);
  console.log(`[BulkData] Built indexes in ${indexTime}s:`);
  console.log(`  - ${nameIndex.size} unique card names`);
  console.log(`  - ${oracleIdIndex.size} oracle IDs`);
  console.log(`  - ${rulingsIndex.size} cards with rulings`);
}
...
function parseQuery(query) {
  ...
  parts.forEach(part => {
    ...
    if (part.match(/^(c|color)([:=<>!]{1,2})/i)) {
      ...
      if (match) {
        ...
        if (/^\d+$/.test(value)) {
          ...
          // Pre-convert to uppercase for faster filterCards processing
          filters.colorsUpper = filters.colors.map(c => c.toUpperCase());
    ...
    if (part.match(/^(id|identity)([:=<>!]{1,2})/i)) {
      ...
      if (match) {
        ...
        if (/^\d+$/.test(value)) {
          ...
          // Pre-convert to uppercase for faster filterCards processing
          filters.colorIdentityUpper = filters.colorIdentity.map(c => c.toUpperCase());
    ...
    if (part.match(/^t:/i) && !part.match(/^t:id:/i)) {
...
function filterCards(cards, filters) {
  return cards.filter(card => {
    ...
    if (filters.colorsCount !== null) {
      ...
      if (op === '<' && count >= filters.colorsCount) return false;
      if (op === '<=' && count > filters.colorsCount) return false;
    } else if ((filters.colorsUpper && filters.colorsUpper.length > 0) || filters.colors.length > 0) {
      // Note: Scryfall API returns card.colors as uppercase ['W', 'U', 'B', 'R', 'G']
      // We compare directly without conversion for performance
      // Use colorsUpper if available (pre-converted), otherwise fallback to colors
      const queryColors = filters.colorsUpper || filters.colors.map(c => c.toUpperCase());
      const cardColors = card.colors || [];
    ...
    if (filters.colorIdentityCount !== null) {
      ...
      if (op === '<' && count >= filters.colorIdentityCount) return false;
      if (op === '<=' && count > filters.colorIdentityCount) return false;
    } else if ((filters.colorIdentityUpper && filters.colorIdentityUpper.length > 0) || filters.colorIdentity.length > 0) {
      // Note: Scryfall API returns card.color_identity as uppercase ['W', 'U', 'B', 'R', 'G']
      // We compare directly without conversion for performance
      // Use colorIdentityUpper if available (pre-converted), otherwise fallback to colorIdentity
      const queryIdentity = filters.colorIdentityUpper || filters.colorIdentity.map(c => c.toUpperCase());
      const cardIdentity = card.color_identity || [];
...
async function getRandomCard(query = '', suppressLog = false) {
  ...
  // Deduplicate by oracle_id to avoid skewing results with heavily reprinted cards
  // This ensures we get unique cards (by gameplay identity), not just unique printings
  // Matches the behavior of Scryfall's is:unique filter
  const uniqueCardsByOracleId = new Map();
  for (const card of matchingCards) {
    if (card.oracle_id) {
      // Keep the first card we encounter for each oracle_id
      if (!uniqueCardsByOracleId.has(card.oracle_id)) {
        uniqueCardsByOracleId.set(card.oracle_id, card);
      }
    } else {
      // Cards without oracle_id are rare but can exist (e.g., some tokens or special cards)
      // We'll include them using their card id as a fallback key to avoid losing them
      if (card.id && !uniqueCardsByOracleId.has(card.id)) {
        uniqueCardsByOracleId.set(card.id, card);
      }
    }
  }
  matchingCards = Array.from(uniqueCardsByOracleId.values());
  
  // Log filter results for debugging (unless suppressed)
  if (query && !suppressLog) {
    console.log(`[BulkData] Query: "${query}" matched ${matchingCards.length} unique cards`);
 ...
 * Now uses O(1) index lookup instead of O(n) linear search
 */
function getCardByName(name, set = null) {
  if (!cardsLoaded || !cardsDatabase || !nameIndex) {
  ...
  const indexedCards = nameIndex.get(normalized);
  if (indexedCards && indexedCards.length > 0) {
    const setCode = set ? set.toLowerCase() : null;
    // If set specified, filter by set
    if (setCode) {
      const setMatch = indexedCards.find(c => c.set === setCode);
  ...
  // Fallback to exact match on original name (edge case for non-normalized names)
 ...
 * Now uses O(1) index lookup instead of O(n) filter
 */
function getRulings(oracleId) {
  if (!INCLUDE_RULINGS) {
    return [];
  }
  if (!cardsLoaded || !rulingsIndex) {
    return [];
  }
  // O(1) indexed lookup
  return rulingsIndex.get(oracleId) || [];
}

/**
 * Schedule automatic update check
 * @param {number} delay - Delay in milliseconds before running the update (default: UPDATE_INTERVAL)
 */
function scheduleUpdateCheck(delay = UPDATE_INTERVAL) {
  ...
  updateTimeout = setTimeout(async () => {
    try {
      ...
      // Pass false to prevent duplicate timer scheduling (we reschedule below)
      cardsLoaded = false;
      cardsDatabase = null;
      rulingsDatabase = null;
      
      try {
        await loadBulkData(false);  // Don't schedule - we handle it here
        console.log('[BulkData] Scheduled update complete!');
        // Reschedule next update after successful completion (24 hours)
        scheduleUpdateCheck();
      } catch (reloadError) {
        ...
        // Still reschedule with normal interval (we have old data)
        scheduleUpdateCheck();
      }
    } catch (error) {
      console.error('[BulkData] Scheduled update failed:', error.message);
      // Retry in 1 hour if download fails
      console.log(`[BulkData] Will retry update in ${RETRY_INTERVAL / 1000 / 60} minutes`);
      scheduleUpdateCheck(RETRY_INTERVAL);
    }
  }, delay);
}

/**
 * Stop automatic update checks and cleanup timers
 */
function stopUpdateCheck() {
  if (updateTimeout) {
    clearTimeout(updateTimeout);
    updateTimeout = null;
    console.log('[BulkData] Stopped automatic update checks');
  }
}
