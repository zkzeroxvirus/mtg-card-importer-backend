...
// Include singular/plural forms like phenomenon/phenomena to match Scryfall type lines.
const EXTRA_CARD_TYPES = ['token', 'emblem', 'scheme', 'plane', 'phenomenon', 'phenomena', 'vanguard', 'conspiracy'];
const EXTRA_CARD_TYPE_SET = new Set(EXTRA_CARD_TYPES);
const EXTRA_CARD_TYPE_PATTERN = EXTRA_CARD_TYPES.join('|');
// Type lines use em-dash/en-dash separators; include both Unicode variants.
const EXTRA_CARD_TYPE_REGEX = new RegExp(
  `(?:^|[\\s\\u2014\\u2013])(${EXTRA_CARD_TYPE_PATTERN})(?:[\\s\\u2014\\u2013]|$)`,
  'i'
);
...
async function getRandomCard(query = '', suppressLog = false) {
  ...
  // parseQuery only captures t: filters, so still scan the raw query for type:/is: extras.
  const hasExtraTypeFilter = (filters.type || []).some(type => EXTRA_CARD_TYPE_SET.has(type));
  const includeExtraTypes = hasExtraTypeFilter || EXTRA_TYPE_QUERY_REGEX.test(query);
 ...
 * Returns null if bulk data not loaded (signals API fallback required)
 */
async function searchCards(query = '', limit = 10, suppressLog = false) {
  if (!cardsLoaded || !cardsDatabase) {
    return null;  // Signal API fallback required
  }
