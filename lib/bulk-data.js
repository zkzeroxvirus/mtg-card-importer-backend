...
function parseQuery(query) {
  ...
  parts.forEach(part => {
    ...
    // EUR price filter
    if (part.match(/^eur[:=<>]/i)) {
      if (part.match(/eur>=([0-9.]+)/i)) {
        const match = part.match(/eur>=([0-9.]+)/i);
        filters.eurGreaterEqual = parseFloat(match[1]);
      } else if (part.match(/eur<=([0-9.]+)/i)) {
        const match = part.match(/eur<=([0-9.]+)/i);
        filters.eurLessEqual = parseFloat(match[1]);
      } else if (part.match(/eur>([0-9.]+)/i)) {
        const match = part.match(/eur>([0-9.]+)/i);
        filters.eurGreater = parseFloat(match[1]);
      } else if (part.match(/eur<([0-9.]+)/i)) {
        const match = part.match(/eur<([0-9.]+)/i);
        filters.eurLess = parseFloat(match[1]);
      } else if (part.match(/eur[:=]([0-9.]+)/i)) {
        const match = part.match(/eur[:=]([0-9.]+)/i);
        filters.eurEquals = parseFloat(match[1]);
      }
    }
    
    // TIX price filter (MTGO tickets)
    if (part.match(/^tix[:=<>]/i)) {
      if (part.match(/tix>=([0-9.]+)/i)) {
        const match = part.match(/tix>=([0-9.]+)/i);
        filters.tixGreaterEqual = parseFloat(match[1]);
      } else if (part.match(/tix<=([0-9.]+)/i)) {
        const match = part.match(/tix<=([0-9.]+)/i);
        filters.tixLessEqual = parseFloat(match[1]);
      } else if (part.match(/tix>([0-9.]+)/i)) {
        const match = part.match(/tix>([0-9.]+)/i);
        filters.tixGreater = parseFloat(match[1]);
      } else if (part.match(/tix<([0-9.]+)/i)) {
        const match = part.match(/tix<([0-9.]+)/i);
        filters.tixLess = parseFloat(match[1]);
      } else if (part.match(/tix[:=]([0-9.]+)/i)) {
        const match = part.match(/tix[:=]([0-9.]+)/i);
        filters.tixEquals = parseFloat(match[1]);
      }
    }
  });
