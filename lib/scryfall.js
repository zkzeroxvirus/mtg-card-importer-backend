...
let cardImageProxyUrlProvider = null;
...
function setCardImageProxyUrlProvider(provider) {
  cardImageProxyUrlProvider = typeof provider === 'function' ? provider : null;
}
...
function isScryfallImageUrl(urlString) {
  ...
  return /^https?:$/i.test(urlObj.protocol) && urlObj.hostname.toLowerCase() === 'cards.scryfall.io';
}

function proxyCardImageUrl(urlString) {
  if (typeof urlString !== 'string') {
    return urlString;
  }

  const normalized = urlString.trim();
  if (!isScryfallImageUrl(normalized)) {
    return normalized;
  }

  if (cardImageProxyUrlProvider) {
    const proxied = cardImageProxyUrlProvider(normalized);
...
function convertToTTSCard(scryfallCard, backUrl, position = null) {
  const imageUrl = proxyCardImageUrl(getCardImageUrl(scryfallCard));
  if (!imageUrl) {
  ...
  const customDeckEntry = {
    FaceURL: imageUrl,
    BackURL: proxyCardImageUrl(backUrl),
    NumWidth: 1,
    NumHeight: 1,
    BackIsHidden: true,
    UniqueBack: false,
    ...
    if (backFaceImageUrl) {
      ...
      ttsCard.States = {
        2: {
          ...
          CustomDeck: {
            '2': {
              FaceURL: proxyCardImageUrl(backFaceImageUrl),
              BackURL: proxyCardImageUrl(backUrl),
              NumWidth: 1,
              NumHeight: 1,
              BackIsHidden: true,
