 ...
 * Optional order parameter matches Scryfall search ordering (e.g., 'random').
 */
async function searchCards(query, limit = 10, unique = 'cards', order = null) {
  ...
  try {
    ...
    const tokenQueryRegex = /\b(?:t|type|is):token\b/i;
    const dfcQueryRegex = /\b-?is:dfc\b/i;
    if (tokenQueryRegex.test(query) && !dfcQueryRegex.test(query)) {
      finalQuery = `${query} -is:dfc`;
    }

    let url = `/cards/search?q=${encodeURIComponent(finalQuery)}&unique=${encodeURIComponent(unique)}`;
    if (order) {
      url += `&order=${encodeURIComponent(order)}`;
    }
    const response = await withRetry(() => 
      scryfallClient.get(url)
    );
    const cards = response.data.data || [];
