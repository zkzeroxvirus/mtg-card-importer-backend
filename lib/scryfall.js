...
function isNonPlayableCard(card) {
  ...
  if (['memorabilia', 'token', 'minigame'].includes(setType)) {
    return true;
  }
...
async function searchCards(query, limit = 10, unique = 'cards', order = null) {
  try {
    ...
    const cards = [];
    let nextUrl = `/cards/search?${params.toString()}`;

    while (nextUrl && cards.length < limit) {
      await globalQueue.wait();

      const response = await withRetry(() => scryfallClient.get(nextUrl));
      ...
      try {
        const nextPageUrl = new URL(response.data.next_page);
