...
class GlobalRequestQueue {
  ...
  async processQueue() {
    ...
    try {
      // Process only one request, then yield control back to the event loop
      // This prevents CPU busy-waiting when many requests are queued
      const now = Date.now();
      ...
      // Guard against race condition where queue becomes empty
      if (resolve && typeof resolve === 'function') {
        resolve();
      }
    } catch (error) {
      // Log error but don't crash - continue processing queue
      ...
      // using setImmediate to yield control to the event loop
...
let scryfallClient;
let scryfallClientFactory;
function getScryfallClient() {
  // In Jest tests axios.create is reassigned to a new mock function per case; refresh the client when its reference changes.
  const createFn = axios.create;
  if (!scryfallClient || (process.env.NODE_ENV === 'test' && createFn !== scryfallClientFactory)) {
    scryfallClientFactory = createFn;
    scryfallClient = axios.create({
    ...
    try {
      ...
      if (status === 429) {
        ...
        if (retryAfter) {
          ...
          const retryAfterMs = isNaN(retryAfter) ? 60000 : parseInt(retryAfter) * 1000;
          // Ensure minimum delay of 1 second even if Retry-After says 0
          delayMs = Math.max(retryAfterMs, 1000);
        } else {
...
async function fetchTokenByType(name) {
  ...
  const response = await withRetry(() =>
    getScryfallClient().get(`/cards/search?q=${query}&unique=cards`)
  );
...
async function getCard(name, set = null) {
  ...
  try {
    ...
    return await withRetry(() => getScryfallClient().get(url).then(r => r.data));
  } catch (error) {
...
async function autocompleteCardName(query) {
  ...
  try {
    ...
    const response = await withRetry(() =>
      getScryfallClient().get(`/cards/autocomplete?q=${encodeURIComponent(normalizedQuery || query)}`)
    );
...
async function getCardById(id) {
  ...
  try {
    return await withRetry(() => 
      getScryfallClient().get(`/cards/${id}`).then(r => r.data)
    );
  } catch (error) {
    if (error.response?.status === 404) {
...
async function getCardBySetNumber(set, number, lang = 'en') {
  ...
  try {
    return await withRetry(() => 
      getScryfallClient().get(`/cards/${set}/${number}/${lang}`).then(r => r.data)
    );
  } catch (error) {
    if (error.response?.status === 404) {
...
async function searchCards(query, limit = 10, unique = 'cards') {
  ...
  try {
    ...
    const response = await withRetry(() => 
      getScryfallClient().get(`/cards/search?q=${encodeURIComponent(finalQuery)}&unique=${encodeURIComponent(unique)}`)
    );
    const cards = response.data.data || [];
...
/**
 * Build filter query to exclude non-playable cards
 * This matches the filtering behavior of bulk-data.js isNonPlayableCard()
 * Excludes: test cards, acorn stamps, digital-only, oversized, tokens, emblems, basic lands, etc.
 * Also filters to unique cards (by oracle_id) to avoid duplicate printings and alternative arts
 */
function buildNonPlayableFilter() {
  const filters = [
    'is:unique',      // Return only unique cards (one per oracle_id), avoiding reprints and alternative arts
    '-set:cmb1',      // Mystery Booster playtest cards
    '-set:cmb2',      // Mystery Booster playtest cards  
    '-set:mb2',       // Mystery Booster Convention Edition playtest cards
    'game:paper',     // Only cards available in paper (excludes digital-only)
    '-is:oversized',  // Exclude oversized cards
    '-stamp:acorn',   // Exclude acorn-stamped cards (casual/Un-sets)
    '-t:basic',       // Exclude basic lands
    // Exclude non-playable layouts
    '-layout:token',
    ...
    '-layout:attraction',
    '-layout:stickers',
    // Exclude funny/memorabilia/token/minigame set types
    '-set_type:funny',
...
/**
 * Check if a card is non-playable (test card, token, emblem, etc.)
 * This is used as a validation check on cards returned from Scryfall
 * Matches the logic in bulk-data.js isNonPlayableCard()
 */
function isNonPlayableCard(card) {
  ...
  // Exclude meld result cards (collector numbers ending in 'b')
...
async function getRandomCard(query = '', suppressLog = false) {
  ...
  // Add non-playable card filters to the query
  // This ensures test cards, tokens, emblems, etc. are excluded from random results
  const nonPlayableFilter = buildNonPlayableFilter();
  const finalQuery = query ? `${query} ${nonPlayableFilter}` : nonPlayableFilter;

  let url = `/cards/random?q=${encodeURIComponent(finalQuery)}`;
  ...
  try {
    return await withRetry(() => 
      getScryfallClient().get(url).then(r => r.data)
    );
  } catch (error) {
...
async function getSet(setCode) {
  ...
  try {
    return await withRetry(() => 
      getScryfallClient().get(`/sets/${setCode}`).then(r => r.data)
    );
  } catch (error) {
    if (error.response?.status === 404) {
...
async function getCardRulings(cardName) {
  try {
    ...
    const response = await withRetry(() => 
      getScryfallClient().get(`/cards/${encodeURIComponent(card.id)}/rulings`)
    );
    return response.data.data || [];
  } catch (error) {
    if (error.response?.status === 404) {
...
async function getTokens(cardName) {
  ...
  try {
      ...
      for (const part of tokenParts) {
        try {
          ...
          const tokenData = await withRetry(() => 
            getScryfallClient().get(part.uri).then(r => r.data)
          );
    ...
    console.debug('Token lookup via all_parts failed, falling back to search:', err.message);
  }
  ...
  try {
    ...
    const typeResponse = await withRetry(() => 
      getScryfallClient().get(`/cards/search?q=${encodeURIComponent(typeQuery)}&unique=cards&order=released&dir=desc`)
    );
    ...
    const createResponse = await withRetry(() => 
      getScryfallClient().get(`/cards/search?q=${encodeURIComponent(createQuery)}&unique=cards&order=released&dir=desc`)
    );
    ...
    console.debug('Token search failed:', error.message);
    return [];
  }
}
...
async function getPrintings(cardName) {
  ...
  try {
    ...
    const cardResponse = await withRetry(() => 
      getScryfallClient().get(`/cards/named?fuzzy=${encodeURIComponent(cardName)}`)
    );
    ...
    const response = await withRetry(() => 
      getScryfallClient().get(`/cards/search?q=oracleid:${encodeURIComponent(oracleId)}&unique=prints&order=released`)
    );
