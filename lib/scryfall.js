...
async function getCard(name, set = null) {
  ...
  try {
    const response = await withRetry(() => scryfallClient.get(url));
...
async function autocompleteCardName(q) {
  try {
    await globalQueue.wait();
    const response = await withRetry(() => scryfallClient.get(`/cards/autocomplete?q=${encodeURIComponent(q)}`));
...
async function getCardById(id) {
  await globalQueue.wait();
  try {
    const response = await withRetry(() => scryfallClient.get(`/cards/${id}`));
    return response.data;
  } catch (error) {
    if (error.response?.status === 404) {
...
async function getCardBySetNumber(set, number, lang = null) {
...
async function searchCards(q, limit = 1, unique = 'cards', order = null) {
  const results = [];
...
async function getRandomCard(q = '') {
...
async function getSet(setCode) {
  await globalQueue.wait();
  const response = await withRetry(() => scryfallClient.get(`/sets/${setCode}`));
  return response.data;
}

async function getCardRulings(id) {
  await globalQueue.wait();
  const response = await withRetry(() => scryfallClient.get(`/cards/${id}/rulings`));
  return response.data.data;
}

async function getPrintings(name) {
  await globalQueue.wait();
  const cardResponse = await withRetry(() => scryfallClient.get(`/cards/named?fuzzy=${encodeURIComponent(name)}`));
  const oracleId = cardResponse.data.oracle_id;

  await globalQueue.wait();
  const printingsResponse = await withRetry(() => scryfallClient.get(`/cards/search?q=oracleid:${oracleId}&unique=prints`));
