...
export async function autocompleteCards(q) {
  await limiter.wait();
  const { data } = await axios.get(`${SCRYFALL_BASE}/cards/autocomplete`, { params: { q } });
  return data.data || [];
}
...
export async function getSet(code) {
  await limiter.wait();
  const { data } = await axios.get(`${SCRYFALL_BASE}/sets/${code}`);
  return data;
}

export async function getRulingsById(id) {
  await limiter.wait();
  const { data } = await axios.get(`${SCRYFALL_BASE}/cards/${id}/rulings`);
  return data.data || [];
}

export async function getCollection(identifiers) {
  await limiter.wait();
  const { data } = await axios.post(`${SCRYFALL_BASE}/cards/collection`, { identifiers });
  return data.data || [];
}

export async function getCatalog(type) {
  await limiter.wait();
  const { data } = await axios.get(`${SCRYFALL_BASE}/catalog/${type}`);
  return data.data || [];
}
