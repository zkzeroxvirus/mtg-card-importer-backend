...
async function withRetry(requestFn, maxRetries = 3) {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      console.warn(
        `Scryfall API error (${status}), retrying in ${delayMs}ms (attempt ${attempt + 1}/${maxRetries - 1})`
      );
...
async function getCard(name, set = null) {
  ...
  // Auto-alias common token names by fetching token cards directly instead of appending "token" to the name
  const tokenAliases = ['treasure', 'clue', 'food', 'emblem', 'cub', 'elf', 'goblin', 'zombie', 'construct'];
  const lowerName = name.toLowerCase().trim();

  if (tokenAliases.includes(lowerName) && !lowerName.includes('token')) {
    const tokenCard = await fetchTokenByType(lowerName);
    if (tokenCard) {
      return tokenCard;
    }
  }
  ...
  try {
    ...
    // If fuzzy lookup failed, try a final token-by-type search before erroring
    if (error.response?.status === 404) {
      const tokenCard = await fetchTokenByType(name);
      if (tokenCard) {
        return tokenCard;
      }
...
async function searchCards(query, limit = 10) {
  ...
  try {
    const response = await withRetry(() => 
      scryfallClient.get(`/cards/search?q=${encodeURIComponent(query)}&unique=prints`)
    );
    const cards = response.data.data || [];
...
async function getCardRulings(cardName) {
  ...
  try {
    const response = await withRetry(() => 
      scryfallClient.get(`/cards/named/${encodeURIComponent(cardName)}/rulings`)
    );
...
async function getTokens(cardName) {
  ...
  try {
    // First, try to find tokens by type/name (e.g., t:token name:bird)
    const typeQuery = `t:token name:${encodeURIComponent(cardName)}`;
    const typeResponse = await withRetry(() => 
      scryfallClient.get(`/cards/search?q=${typeQuery}&unique=cards`)
    );
    
    if (typeResponse.data && typeResponse.data.data && typeResponse.data.data.length > 0) {
      return typeResponse.data.data.filter(card => card.type_line && card.type_line.includes('Token'));
    }
    ...
    const createResponse = await withRetry(() => 
      scryfallClient.get(`/cards/search?q=${encodeURIComponent(createQuery)}&unique=cards`)
    );
    
    if (createResponse.data && createResponse.data.data) {
      return createResponse.data.data.filter(card => card.type_line && card.type_line.includes('Token'));
    }
