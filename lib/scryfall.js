...
async function getCard(name, set = null) {
  ...
  // Token-first: if the query looks like a token request, try type-based token lookup before fuzzy card search
  const tokenAliases = ['treasure', 'clue', 'food', 'emblem', 'cub', 'elf', 'goblin', 'zombie', 'construct'];
  const lowerName = name.toLowerCase().trim();
  const strippedToken = lowerName.replace(/\s*token\s*$/, '').trim();

  if (lowerName.includes('token') || tokenAliases.includes(lowerName) || tokenAliases.includes(strippedToken)) {
    const tokenLookupName = tokenAliases.includes(strippedToken) ? strippedToken : strippedToken || lowerName;
    const tokenCard = await fetchTokenByType(tokenLookupName);
    if (tokenCard) {
      return tokenCard;
    }
  }
