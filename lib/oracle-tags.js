...
function shouldRefreshOracleTagsFile(forceRefresh) {
  return forceRefresh
    || !fs.existsSync(ORACLE_TAGS_FILE)
    || !readJsonFile(ORACLE_TAGS_MANIFEST_FILE);
}
...
async function loadOracleTags(options = {}) {
  ...
  loadPromise = (async () => {
    ...
    if (shouldRefreshOracleTagsFile(forceRefresh)) {
      try {
        ...
        console.warn(`[OracleTags] Failed to refresh oracle tags bulk data, using existing cache: ${error.message}`);
      }
    }
