...
function calculateWorkerCount(options = {}) {
  ...
  const useBulkDataValue = options.useBulkData ?? process.env.USE_BULK_DATA ?? 'false';
  const useBulkData = String(useBulkDataValue).toLowerCase() === 'true';
  ...
  const memoryPerWorkerMB = useBulkData
